...
# =====================
    ...
    try:
        ...
        if len(df) >= 20:
            ...
            with g1:
                st.plotly_chart(
                    donut_chart("📊 趨勢分析表", trend_score, trend_text, "#ffcc00"),
                    use_container_width=True
                )

            with g2:
                st.plotly_chart(
                    donut_chart("📦 量能表", volume_score, volume_status, "#00e5ff"),
                    use_container_width=True
                )

            with g3:
                st.plotly_chart(
                    donut_chart("🤖 人工智慧預測", ai_score, ai_text, ai_color),
                    use_container_width=True
                )
...
with b1:
    ...
    st.markdown(
        ...
        f"<p style='font-size:24px; color:{curr_color}; font-weight:bold;'>"
